...
export class Estudiante{
    ...
    /**
     * @function
     * @returns {Object} Estudiante en formato JSON.
     * @description Devuelve el estudiante en formato JSON.
     */
    toJSON() {
        return {
            id: this.#id,
            ...
            asignaturas: this.#asignaturas.map(a => ({
                asignatura: a[0].toJSON(),
                calificacion: a[1]
            })),
            registros: this.#registros.map(registro => ({
                asignatura: registro[0],
                fecha: registro[1].toISOString(),
                tipo: registro[2]
            }))
        };
    }
     ...
     * @param {String} tipo Tipo del registro (Matrícula o Desmatrícula)
     * @description **Añade** un registro al Array `#registros`.
     */
    añadirRegistro(asignatura, fecha, tipo) {
        this.#registros.push([asignatura, new Date(fecha), tipo]);
    }    
